    ...
    def get(self):
        ...
        if limit:
            try:
                ...
                return Response(json.umps({'message': 'Limit must be an integer between 1 and 50'}), mimetype="application/json", status=400)
            if limit > 50 or limit < 1:
                return Response(json.umps({'message': 'Limit must be an integer between 1 and 50'}), mimetype="application/json", status=400)
        else:
            limit = 10

        posts = posts.order_by(Post.pub_date.desc()).limit(limit).all()

        data = [
            item.to_dict() for item in posts
        ]
